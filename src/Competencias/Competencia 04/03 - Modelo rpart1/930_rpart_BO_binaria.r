...
PARAM$experimento <- "HT930_compFinal_modelo03"
...
dataset  <- fread(paste0("exp/", PARAM$exp_input, "/", PARAM$file_name))   #donde entreno

#creo la clase_binaria  SI= {BAJA+1, BAJA+2}  NO={CONTINUA}
dataset[ foto_mes==PARAM$ultimo_mes_clase, clase_binaria :=  ifelse( clase_ternaria=="CONTINUA", "NO", "SI" ) ]

#defino los datos donde entreno
dtrain  <- dataset[ foto_mes==PARAM$ultimo_mes_clase, ]

new.wd <- paste0("./exp/", PARAM$experimento)

#creo la carpeta donde va el experimento
# HT  representa  Hiperparameter Tuning
dir.create( new.wd, showWarnings = FALSE )
setwd(new.wd)   #Establezco el Working Directory DEL EXPERIMENTO
