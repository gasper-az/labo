...
PARAM  <- list()
PARAM$experimento <- "TS9320_compFinal_modelo05_public_vs_private"

PARAM$exp_input  <- "FE9250_compFinal_modelo05"

PARAM$future       <- c( 202107 ) # mes a predecir

PARAM$final_train  <- c( 201901, 201902, 201903, 201904, 201905, 201906, 201907, 202101, 202102, 202103, 202104, 202105 )

PARAM$train$training     <- c( 201901, 201902, 201903, 201904, 201905, 202101, 202102, 202103, 202104, 202105 )
PARAM$train$validation   <- c( 201906 )
PARAM$train$testing      <- c( 201907 )

PARAM$train$sampling_total  <- 0.3  # 1.0 significa que NO se hace sampling total,  0.3 es quedarse con el 30% de TODOS los registros
...
PARAM$train$semilla_sampling  <- 763369
PARAM$train$semilla_under     <- 763381
